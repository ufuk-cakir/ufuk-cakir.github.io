...
(function () {
  ...
  /* ---------- helpers ---------- */
  const slug = s => String(s || "").toLowerCase().replace(/[''".,:;()]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 40) || "untitled";
  ...
  function buildFS() {
    ...
    const aboutTxt = [ID.name, ID.role, "", ID.tagline].filter(Boolean).join("\n") + "\n\nLocation: " + (ID.location || "—");
    return {
      kind: "dir",
      name: "~",
      children: {
        "publications": {
          kind: "dir",
          name: "publications",
          children: mkItems(SITE.publications, ".md")
        },
        "projects": {
          kind: "dir",
          name: "projects",
          children: mkItems(SITE.projects)
        },
        "talks": {
          kind: "dir",
          name: "talks",
          children: mkItems(SITE.talks, ".md")
        },
        "writing": {
          kind: "dir",
          name: "writing",
          children: mkItems(SITE.writing, ".md")
        },
        "outreach": {
          kind: "dir",
          name: "outreach",
          children: mkItems(SITE.outreach)
        },
        "about.txt": {
          kind: "file",
          text: aboutTxt
  ...
  function resolvePath(root, cwd, raw) {
    let segs;
    if (!raw || raw === "~" || raw === "/") segs = [];else if (raw.startsWith("~/")) segs = raw.slice(2).split("/");else if (raw.startsWith("/")) segs = raw.slice(1).split("/");else segs = cwd.concat(raw.split("/"));
    const out = [];
    for (const s of segs) {
      if (s === "" || s === ".") continue;
      if (s === "..") {
  ...
  function neofetchLines() {
    ...
    const info = [[(ID.name || "user") + "@cakir-os", ""], ["-----------------", ""], ["OS", "Çakır OS"], ["Host", ID.location || "Oxford"], ["Role", ID.roleShort || ID.role || "—"], ["Shell", "csh (Çakır shell)"], ["Uptime", uptimeJoke], ["Theme", "near-black / phosphor-green"]];
    const out = [];
  ...
  function Terminal() {
    ...
    const [lines, setLines] = useState(() => [{
      c: "green",
      t: "Çakır OS Terminal — v1.0"
    }, {
      c: "dim",
      t: "Welcome, " + (ID.name || "friend") + "."
    }, {
      c: "dim",
      t: "type 'help' to get started."
    }, {
      t: ""
    }]);
    ...
    const run = useCallback(raw => {
      ...
      switch (cmd) {
        case "help":
          ...
          out.push({
            c: "dim",
            t: "tip: try 'ls', then 'cat <file>'. some things are hidden."
          });
          break;
        case "ls":
          {
            ...
            if (!node) {
              err("ls: no such file or directory: " + pathArg);
              break;
            }
            if (node.kind === "file") {
              out.push({
            ...
            names.forEach(nm => {
              ...
              any = true;
              out.push({
                c: ch.kind === "dir" ? "dir" : ch.hidden ? "dim" : "file",
                t: ch.kind === "dir" ? nm + "/" : nm
              });
            });
            if (!any && !showAll) out.push({
              c: "dim",
              t: "(empty)"
            });
            break;
        ...
        case "cat":
          {
            ...
            if (!node || node === root || node.kind === "dir") {
              if (node && node.kind === "dir") err("cat: " + target + ": is a directory");else err("cat: " + target + ": no such file");
              break;
            }
            if (node.secret) {
              node.text.split("\n").forEach(t => out.push({
            ...
            if (node.item) {
              catItem(node.item).forEach(l => out.push(l));
              break;
            }
            out.push({
              c: "dim",
        ...
        case "open":
          {
            const target = args[0];
            if (!target) {
              err("open: usage: open <name>");
              break;
            }
            const segs = resolvePath(root, cwd, target);
            const node = nodeAt(root, segs);
            if (!node || !node.item) {
              err("open: " + target + ": nothing to open");
              break;
            }
            const href = firstHref(node.item);
            if (!href) {
              err("open: " + target + ": this item has no link");
              break;
            }
            try {
            ...
            out.push({
              c: "link",
              t: "opening " + (node.item.title || node.item.name) + " → " + href
            });
            break;
          }
        case "whoami":
        ...
        case "freedoom":
          out.push({
            c: "green",
            t: "Rip and tear... booting FREEDOOM (click the canvas to begin)."
          });
          try {
            window.dispatchEvent(new CustomEvent("os-open", {
              detail: {
                url: "https://archive.org/embed/fdprev-v13",
                title: "FREEDOOM"
              }
            }));
          } catch (e) {}
        ...
        case "sudo":
          out.push({
            c: "err",
            t: "Nice try. This incident will be reported."
          });
          break;
        case "exit":
          out.push({
            c: "dim",
            t: "There is no escape. (close the window like a normal person.)"
          });
          break;
        case "make":
          if (rest.toLowerCase() === "me a sandwich") out.push({
            c: "dim",
            t: "What? Make it yourself. (try: sudo make me a sandwich)"
          });else out.push({
            c: "err",
            t: "make: nothing to be done for '" + rest + "'"
          });
          break;
        default:
    ...
    const complete = useCallback(val => {
      ...
      if (names.length === 1) {
        const nm = names[0];
        return {
          value: head + dirPart + nm + (dirNode.children[nm].kind === "dir" ? "/" : " ")
        };
      }
      if (names.length > 1) {
        const p = lcp(names);
        return p.length > namePart.length ? {
          value: head + dirPart + p
        } : {
          list: names.map(disp)
        };
      }
    ...
    const onKeyDown = useCallback(e => {
      ...
      if (e.key === "Tab") {
        ...
        if (r.value != null) setValue(r.value);else if (r.list && r.list.length) push([{
          c: "prompt",
          t: promptPath(cwd) + " $ " + value
        }, {
          c: "dim",
          t: r.list.join("    ")
        }]);
    ...
    const cls = c => "term-line" + (c === "err" ? " term-err" : c === "dir" ? " term-dir" : c === "file" ? " term-file" : c === "dim" ? " term-dim" : c === "h" ? " term-h" : c === "link" ? " term-link" : c === "green" ? " term-green" : c === "prompt" ? " term-promptline" : "");
    return /*#__PURE__*/React.createElement("div", {
      className: "win-body app-term",
      ref: rootRef,
      onClick: focus
      ...
      className: cls(ln.c),
      key: i
    }, ln.t === "" ? " " : ln.t))), /*#__PURE__*/React.createElement("div", {
      className: "term-inputrow",
      onClick: focus
    }, /*#__PURE__*/React.createElement("span", {
      className: "term-ps1"
    }, promptPath(cwd), " $"), /*#__PURE__*/React.createElement("label", {
      className: "term-inputwrap"
    }, /*#__PURE__*/React.createElement("span", {
      className: "term-shown"
